  ...
  provisioner "file" {
    content = templatefile(
      "${path.module}/files/hostvars_template.yml",
      {
        ...
        appserver_host_ip      = var.jumphost_private_ip[count.index] # the ip address that the jumphost has on the public subnet
    ...
    connection {
      ...
      host        = var.jumphost_public_ip[count.index]
    }
  }
}
